...
def postprocess_40m(cfg, duration_sec=600, create_full_study=False):
    ...
    SortingTrue.save(folder=GT_40m_PATH, verbose=True, overwrite=True, n_jobs=-1, chunk_duration="1s", progress_bar=True)
    logger.info(f"Completed in %s", round(time() - t0, 1))
    
    # save study for 10 minute of GT SortingExtractor    
    create_study(SortingTrue, Prep, STUDY_40M_PATH)
...
postprocess_40m(cfg, duration_sec=40*60, create_full_study=True)