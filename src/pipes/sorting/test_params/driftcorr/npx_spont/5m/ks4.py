...
Resources: takes 130 GB RAM, 21 GB of VRAM, 100% GPU-Util
...
Usage: 
    ...
    3. Run the script with appropriate command-line arguments.
        ...
        nohup python -m src.pipes.sorting.test_params.driftcorr.npx_spont.5m.ks4 \
            --recording-path dataset/00_raw/recording_npx_spont \
                --preprocess-path dataset/01_intermediate/preprocessing/recording_npx_spont \
                    --sorting-path-corrected ./temp/npx_spont/SortingKS4_5m_RTX5090_DriftCorr \
                        --sorting-output-path-corrected ./temp/npx_spont/KS4_output_5m_RTX5090_DriftCorr/ \
                            --study-path-corrected ./temp/npx_spont/study_ks4_5m_RTX5090_DriftCorr/ \
                                --sorting-path-not-corrected ./temp/npx_spont/SortingKS4_5m_RTX5090_NoDriftCorr \
                                    --sorting-output-path-not-corrected ./temp/npx_spont/KS4_output_5m_RTX5090_NoDriftCorr/ \
                                        --study-path-not-corrected ./temp/npx_spont/study_ks4_5m_RTX5090_NoDriftCorr/ \
                                            --remove-bad-channels

Notes:
- Requires a lot more RAM than Kilosort3 (> 120 GB) for 10 min.

- 1 min recording - 202 sites - batch_size=10000 - takes 20GB VRAM  # OK
- 10 min recording - 202 sites - batch_size=500 - takes 11GB VRAM   # fails - no spike found - tensor shape error
- 5 min recording - 202 sites - batch_size=5000 - takes 11GB VRAM   # OK
- 10 min recording - 202 sites - batch_size=5000 - takes 11GB VRAM  # OK
- 10 min recording - 202 sites - batch_size=2500 - takes 7GB VRAM   # OK
- 5 min recording - 303 sites - batch_size=5000 - takes 11GB VRAM   # OK - manually removed channels (max 9G left)
- 5 min recording - 343 sites - batch_size=5000 -    # KO
- 5 min recording - 343 sites - batch_size=2000 -    # KO

TODO:
- Reduce sampling frequency from 40KHz to 20KHz (KO)
- Reduce recording duration from 10 min to 2.5 min (KO)
- Remove a subset of bad channels
"""
