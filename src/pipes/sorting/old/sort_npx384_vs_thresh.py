...
KS3_SORTING_VS_THRESH_PATHS = param_conf["fig_accuracy_vs_thresh"]["sorting"]
...
for ix, thresh_i in enumerate(KS3_SORTING_VS_THRESH_PATHS):
    
    t0 = time()

    # set ks3 output and SortingExtractor paths
    KS3_OUTPUT_PATH = KS3_SORTING_VS_THRESH_PATHS[thresh_i]["ks3_output"]
    KS3_SORTING_PATH = KS3_SORTING_VS_THRESH_PATHS[thresh_i]["output"]

    # get threshold
    threshold = KS3_SORTING_VS_THRESH_PATHS[thresh_i]["thresh"]

    # sort
    sorting_KS3 = ss.run_sorter(sorter_name='kilosort3', recording=Preprocessed, output_folder=KS3_OUTPUT_PATH, verbose=True, detect_threshold=threshold)

    # remove empty units (Samuel Garcia's advice)
    sorting_KS3 = sorting_KS3.remove_empty_units()
    logger.info(f"Done running kilosort3 for thresh: {threshold} in: %s", round(time() - t0, 1))

    # write
    shutil.rmtree(KS3_SORTING_PATH, ignore_errors=True)
    sorting_KS3.save(folder=KS3_SORTING_PATH)
    logger.info(f"Done saving kilosort3 for thresh: {threshold} in: %s", round(time() - t0, 1))
