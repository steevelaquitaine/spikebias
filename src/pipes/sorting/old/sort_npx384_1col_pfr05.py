"""sort npx384 hex0 simulation with ks3
takes 28 min

usage: 
    sbatch cluster/sort_npx384_hex0.sbatch
...
SIMULATION_DATE = "2023_09_12"    # the run (date)
...
# batch size "NT" must be multiple of 32 + ntbuff. This is the batch size.
# (default 'ntbuff' is 64, the samples of symmetrical buffer for whitening and spike detection)
# default batch size is 65792 timepoints
# we double the batch size to ensure there is at least one spike per batch. Otherwise it crashes.
t0 = time()
sorting_KS3 = ss.run_sorter(sorter_name='kilosort3', recording=Recording, output_folder=KS3_OUTPUT_PATH, verbose=True, NT=65792*2)
