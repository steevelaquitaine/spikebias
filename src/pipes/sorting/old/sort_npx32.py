...
SIMULATION_DATE = "2023_02_19"    # the run (date)
...
t0 = time()
Preprocessed = si.load_extractor(preprocessed_path)
logger.info("Done loading recording in: %s", round(time() - t0, 1))

# run sorting
t0 = time()
sorting_KS3 = ss.run_kilosort3(Preprocessed, output_folder=KS3_OUTPUT_PATH, verbose=True)
