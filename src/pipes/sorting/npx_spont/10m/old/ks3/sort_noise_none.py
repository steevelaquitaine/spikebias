...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks3/sort_noise_none.sbatch
...
# SET PATHS
# trace
WIRED_40M_PATH = data_conf["probe_wiring"]["output_noise_0uV"]
...
KS3_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["output_buttw_noise_0uV"]
KS3_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["ks3_output_buttw_noise_0uV"]
...
logger.info("Done running kilosort3 for 10 min no noise in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS3_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS3_SORTING_PATH)
logger.info("Done saving kilosort3 for 10 min no noise in: %s", round(time() - t0, 1))