...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks3/sort_noise_ftd_gain_ftd_ctf100.sbatch
...
sorter_params = {
    ...
    "sig": 20,
    "freq_min": 100, # 300 (default) -> informed by the PSD we lower the cutoff
    "sigmaMask": 30,
    "lam": 20.0,
    "nPCs": 3,
    "ntbuff": 64,
...
# SET PATHS
# trace
WIRED_40m_PATH = data_conf["probe_wiring"]["40m"]["output_noise_fitd_gain_fitd_int16"]
WIRED_10m_PATH = data_conf["probe_wiring"]["10m"]["output_noise_fitd_gain_fitd_int16"]
# sorter
KS3_PACKAGE_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["input_buttw"]
KS3_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["output_buttw_noise_ftd_gain_ftd_ctf100"]
KS3_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["ks3_output_buttw_noise_ftd_gain_ftd_ctf100"]
...
if SAVE_REC_INT16:
    WiredFloat32 = si.load_extractor(WIRED_40m_PATH)
    WiredFloat16 = spre.astype(WiredFloat32, "int16")
    WiredFloat16.save(
        folder=WIRED_10m_PATH,
        format="binary",
        n_jobs=4,
        chunk_memory="40G",
        overwrite=True,
...
t0 = time()
WiredInt16 = si.load_extractor(WIRED_10m_PATH)
...
logger.info("Done saving kilosort3 with h-filtering cutoff of 100 Hz: %s", round(time() - t0, 1))