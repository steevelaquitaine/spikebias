...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks3/sort_noise_0uV_gain_1e6.sbatch
...
sorter_params = {
    ...
    "nfilt_factor": 4,
    "do_correction": True,
    "NT":  2*65792, # None, #2*1024 + 64, # #65792*3, # None: automatically computed, # 65792
...
# SET PATHS
# trace
WIRED_40M_INT16_PATH = data_conf["probe_wiring"]["40m"]["output_noise_none_gain_1e6_int16"]
WIRED_INT16_PATH = data_conf["probe_wiring"]["10m"]["output_noise_0uV_gain_1e6_int16"]
# sorter
KS3_PACKAGE_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["input_buttw"]
KS3_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["output_buttw_noise_0uV_gain_1e6"]
KS3_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["ks3_output_buttw_noise_0uV_gain_1e6"]
...
logger.info("Done running kilosort3 on Marques 10m noise 0uV gain 1e5 in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS3_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS3_SORTING_PATH)
logger.info("Done saving kilosort3 on Marques 10m noise 0uV gain 1e5 in: %s", round(time() - t0, 1))