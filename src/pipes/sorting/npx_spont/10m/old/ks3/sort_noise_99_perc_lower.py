...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks3/sort_noise_99_perc_lower.sbatch
...
# SET PATHS
# trace
WIRED_40M_PATH = data_conf["probe_wiring"]["40m"]["output_gain_fitd_noise_99_perc_lower"]
WIRED_INT16_PATH = data_conf["probe_wiring"]["10m"]["output_gain_fitd_noise_99_perc_lower_int16"]
# sorter
KS3_PACKAGE_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["input_buttw"]
KS3_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["output_buttw_gain_fitd_noise_99_perc_lower"]
KS3_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["10m"]["ks3_output_buttw_gain_fitd_noise_99_perc_lower"]
...
logger.info("Done running kilosort3 for 10 min noise 99 perc lower in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS3_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS3_SORTING_PATH)
logger.info("Done saving kilosort3 for 10 min noise 99 perc lower in: %s", round(time() - t0, 1))