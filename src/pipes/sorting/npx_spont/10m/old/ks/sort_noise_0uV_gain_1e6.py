...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks/sort_noise_0uV_gain_1e6.sbatch
...
KS_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort"]["10m"]["output_buttw_noise_0uV_gain_1e6_int16"]
KS_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort"]["10m"]["ks_output_buttw_noise_0uV_gain_1e6_int16"]
...
logger.info("Done running kilosort on Marques 10m noise 0uV gain 1e6 in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS_SORTING_PATH)
logger.info("Done saving kilosort on Marques 10m noise 0uV gain 1e6 in: %s", round(time() - t0, 1))