...
usage:
    ...
    sbatch cluster/sorting/marques_silico/10m/buttw/ks/sort_noise_0uV.sbatch
...
# SET PATHS
# trace
WIRED_40M_INT16_PATH = data_conf["probe_wiring"]["40m"]["output_noise_0uV"]
WIRED_INT16_PATH = data_conf["probe_wiring"]["10m"]["output_noise_0uV_int16"]
# sorter
KS_PACKAGE_PATH = data_conf["sorting"]["sorters"]["kilosort"]["input"]
KS_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort"]["10m"]["output_buttw_noise_0uV"]
KS_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort"]["10m"]["ks_output_buttw_noise_0uV"]
...
logger.info("Done running kilosort on Marques 10m noise 0uV in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS_SORTING_PATH)
logger.info("Done saving kilosort on Marques 10m noise 0uV in: %s", round(time() - t0, 1))