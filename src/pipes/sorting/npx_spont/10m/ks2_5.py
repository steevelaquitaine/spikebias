"""sort spont biophy. model with Kilosort 2.5
...
usage:
    ...
    # Submit to cluster
    sbatch cluster/sorting/npx_spont/10m/ks2_5.sh
    
duration:

PARAMETERS:

- We mostly preserve KS2.5 default parameters from spikeinterface 0.100.5
- To capture as many units as possible we set:
    - "minFR"=0 instead of 0.1
    - "minfr_goodchannels"=0 instead of 0.1
- we set "nblocks"=1 (to rigid registration) instead of 5
else Error using gpuArray/gather Encountered unexpected error
during CUDA execution. The CUDA error was:
