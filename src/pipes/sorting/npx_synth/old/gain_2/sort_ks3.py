...
usage: 
    ...
    sbatch cluster/sorting/buccino/gain_2/sort_ks3.sbatch
...
# SET READ PATHS
RECORDING_PATH = data_conf["probe_wiring"]["10m"]["output_gain_2_int16"]

# SET WRITE PATHS
KS3_PACKAGE_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["input_buttw"]
KS3_SORTING_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["output_gain_2_int16"]
KS3_OUTPUT_PATH = data_conf["sorting"]["sorters"]["kilosort3"]["ks3_output_gain_2_int16"]
...
logger.info("Done running kilosort3 on 10m gain=2 Buccino in: %s", round(time() - t0, 1))

# write
shutil.rmtree(KS3_SORTING_PATH, ignore_errors=True)
sorting_KS3.save(folder=KS3_SORTING_PATH)
logger.info("Done saving kilosort3 on 10m gain=2 Buccino in: %s", round(time() - t0, 1))