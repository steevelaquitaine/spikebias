...
modified: 11.03.2024
 ...
 usage:
    ...
    sbatch cluster/processing/marques_silico/process_40m_noise_0uV.sbatch
...
from src.pipes.prepro.marques_silico import concat
...
def wire_probe(Recording, save_metadata:bool, job_dict=job_dict):
    ...
    # get write path
    WRITE_PATH = data_conf["probe_wiring"]["output_noise_0uV"]
...
def preprocess_recording(job_dict: dict, filtering='butterworth'):
    ...
    # write path
    WRITE_PATH = data_conf["preprocessing"]["output"]["trace_file_path_noise_0uV"]

    # track time
    t0 = time.time()
    
    # preprocess and write
    Preprocessed = preprocess.run_noise_0uV(data_conf,
                                  param_conf,
                                  filtering=filtering)
    # save
    shutil.rmtree(WRITE_PATH, ignore_errors=True)
