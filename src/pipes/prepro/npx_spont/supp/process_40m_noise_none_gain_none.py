...
modified: 11.03.2024
 ...
 usage:
    ...
    sbatch cluster/prepro/marques_silico/process_40m_noise_none_gain_none.sbatch
...
OFFSET = False
SCALE_AND_ADD_NOISE = 1  # no gain, no noise (raw data)
WIRE = False              # done once then set to False (1h06)
...
def wire_probe(Recording, save_metadata:bool, job_dict=job_dict):
    ...
    # path
    WRITE_PATH = data_conf["probe_wiring"]["40m"]["output_noise_none_gain_none_int16"]
...
def preprocess_recording(Wired, job_dict: dict, filtering='butterworth', load_wired=False):
    ...
    # write path
    READ_PATH = data_conf["probe_wiring"]["40m"]["output_noise_none_gain_none_int16"]
    WRITE_PATH = data_conf["preprocessing"]["output"]["40m"]["trace_file_path_noise_none_gain_none_int16"]
    ...
    Preprocessed = preprocess.run_butterworth_filtering_noise_none_gain_none(data_conf,
                                                                             param_conf)
    # save
    shutil.rmtree(WRITE_PATH, ignore_errors=True)
...
def run(filtering:str="wavelet"):
    ...
    if PREPROCESS:
        Recording = si.load_extractor(data_conf["probe_wiring"]["40m"]["output_noise_none_gain_none_int16"])
