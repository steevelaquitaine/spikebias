...
modified: 11.03.2024
 ...
 usage:
    ...
    sbatch cluster/prepro/marques_silico/process_40m_noise_ftd_gain_ftd_adj.sbatch
...
OFFSET = True
SCALE_AND_ADD_NOISE = "scale_adj_and_add_noise"
...
def wire_probe(Recording, save_metadata:bool, job_dict=job_dict):
    ...
    # get write path
    WRITE_PATH = data_conf["probe_wiring"]["40m"]["output_noise_fitd_gain_fitd_adj_int16"]
...
def preprocess_recording(job_dict: dict, filtering='butterworth'):
    ...
    """
    # write path
    WRITE_PATH = data_conf["preprocessing"]["output"]["40m"]["trace_file_path_gain_fitd_adj_noise_fitd_int16"]
    ...
    # preprocess
    Preprocessed = preprocess.run_butterworth_filtering_noise_ftd_gain_ftd_adj(data_conf,
                                  param_conf)
    # save
    shutil.rmtree(WRITE_PATH, ignore_errors=True)
