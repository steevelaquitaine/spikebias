...
(simulation of Marques experiment) from raw bbp_workflow simulation files to ready-to-sort 
...
FIT_CAST = True         # done once then set to False
WIRE = True             # done once then set to False
SAVE_METADATA = True    # True to add new metadata to wired probe
PREPROCESS = True       # True to update after adding new metadata
...
def fit_and_cast_as_extractor():
    ...
    # cast (30 secs)
    RecordingExtr = recording.run(data_conf, offset=True, scale_and_add_noise=True)
...
def save_metadata():
    ...
    # takes 26 min
    t0 = time.time()
    logger.info("Starting 'save_metadata'")
