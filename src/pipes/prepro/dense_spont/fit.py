...
usage:
    sbatch cluster/prepro/fitting/horvath/fit_silico_probe1_layer1.sbatch
    sbatch cluster/prepro/fitting/horvath/fit_silico_probe1_layer2_3.sbatch
    sbatch cluster/prepro/fitting/horvath/fit_silico_probe2_layer4.sbatch;
    sbatch cluster/prepro/fitting/horvath/fit_silico_probe2_layer5.sbatch;
    sbatch cluster/prepro/fitting/horvath/fit_silico_probe3_layer6.sbatch;

Prerequisites:
    - Recording extractor for in vivo data contains layer metadata
"""
...
from src.nodes.dataeng.silico.probe_wiring import wire_silico_horvath_probe_1, wire_silico_horvath_probe_2, wire_silico_horvath_probe_3
...
NOISE_VIVO_ON_PLOT = 20     # plot mean noise vivo per layer and look at mean noise as initial guess
X0 = NOISE_VIVO_ON_PLOT * SCALE_X_FOR_SPEED # fit initial parameter
...
def myfun_layer(missing_noise, *args):
    ...
    # - wire
    if probe == "probe_1":
        WithNoise = wire_silico_horvath_probe_1(data_conf, param_conf, WithNoise)
    elif probe == "probe_2":
        WithNoise = wire_silico_horvath_probe_2(data_conf, param_conf, WithNoise)
    elif probe == "probe_3":
        WithNoise = wire_silico_horvath_probe_3(data_conf, param_conf, WithNoise)
    else:
...
def fit_noise_by_layer(
    ...
    Args:
        ...
        probe (str): probe_1, probe_2, probe_3
...
def run(probe: str = "probe_1", layer: str = "L1", gain_adjust: float = 2):
    ...
    Args:
        ...
        - "L1", "L2_3", "L4", "L5", "L6"
        gain_adjust: 
        - manual gain adjustment to re-fit trace amplitude to in vivo after fitting noise, because it decreases after preprocessing
    """

    # track time
    t0 = time.time()
    logging.info(f"start fitting pipeline for probe {probe} - layer {layer}.")

    # get config
    # Horvath data
    data_cfg_v, _ = get_config("vivo_horvath", probe).values()
    PREP_PATH_h_vivo = data_cfg_v["preprocessing"]["output"]["trace_file_path"]
    
    # silico
    data_cfg_s, param_cf_s = get_config("silico_horvath", "concatenated/"+probe).values()
    RAW_PATH_sili = data_cfg_s["dataeng"]["campaign"]["output"]["trace_file_path"]
    BLUECONFIG = data_cfg_s["dataeng"]["blueconfig"]
    MISSING_NOISE_PATH = data_cfg_s["preprocessing"]["fitting"]["fitted_noise"]
    FIT_HISTORY_PATH = MISSING_NOISE_PATH + probe + "_" + layer + "_" + "fit_history"
    ...
    # - wire probe and add metadata
    if probe == "probe_1":
        WiredSilicoRecording = wire_silico_horvath_probe_1(data_cfg_s, param_cf_s, Recording)
    elif probe == "probe_2":
        WiredSilicoRecording = wire_silico_horvath_probe_2(data_cfg_s, param_cf_s, Recording)
    elif probe == "probe_3":
        WiredSilicoRecording = wire_silico_horvath_probe_3(data_cfg_s, param_cf_s, Recording)                
    else:
        raise NotImplementedError()
    ...
    # calculate gain between preprocessed in vivo and raw in silico
    gain = (np.absolute(traces_vivo).max() / np.absolute(silico_traces).max()) * gain_adjust

    logging.info("calculated best fit gain - done")

    # get site layer metadata
    ...
    missing_noise = fit_noise_by_layer(
        ...
        FIT_HISTORY_PATH, gain, data_cfg_s, param_cf_s, probe
    )
