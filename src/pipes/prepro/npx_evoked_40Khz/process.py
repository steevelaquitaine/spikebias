...
run: 40m noise ftd gain ftd adj 10 perc less
 ...
 usage:
    ...
    sbatch cluster/prepro/npx_evoked_40Hz/process.sh
...
def run(filtering: str="wavelet"):
    ...
    if WIRE:
        preprocess.wire_probe(data_conf=data_conf,
                              ...
                              save_metadata=SAVE_METADATA, # regions and layers
                              job_dict=job_dict,
                              n_sites=384,
                              load_atlas_metadata=False, # write
                              load_filtered_cells_metadata=False) # write
