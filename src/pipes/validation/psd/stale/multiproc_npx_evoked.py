...
simulation of neuropixels recording in the evoked regime
...
Usage:
    ...
    python src/pipes/validation/psd/multiproc_npx_evoked.py
"""
...
# setup load paths
RAW_PATH = os.path.join(PROJ_PATH, "dataset/00_raw/recording_npx_evoked/")

# setup save paths
RAW_PSD_PATH = os.path.join(PROJ_PATH, "dataset/01_intermediate/psd_raw_npx_evoked.npy")
...
FILT_WINDOW = "hann"
FILT_WIND_NE = SF_NE # 1Hz resolution
FILT_OVERL_NE = int(
    FILT_WIND_NE // 1.5
)
...
def main():
    ...
    IN_CTX = np.isin(sites, layers)
    sites = np.where(IN_CTX)[0]
    ...
    # Calculate psd
    out_raw = get_welch_psd_ne_parallelized(raw_traces)
