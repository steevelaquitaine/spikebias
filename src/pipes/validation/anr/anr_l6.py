...
def main(rank, n_ranks):
    ...
    sites_hs3 = np.where(np.isin(site_ly_hs3, lyrs))[0]

    # vivo
    # probe 3
    site_ly_hv3 = RecHV3.get_property("layers")
    sites_hv3 = np.where(np.isin(site_ly_hv3, lyrs))[0]
    ...
    if rank == 0:
        ...
        mean_hv3, ci_hv3, _ = amp.get_snr_pdfs(snr_hv3, bins)
    elif rank == 4:
        mean_hs3, ci_hs3, _ = amp.get_snr_pdfs(snr_hs3, bins)
    ...
    if rank == 0:
        ...
        # save
        plt.savefig("figures/0_validation/3_anr/fig2q_anr_full_l6.svg", **savefig_cfg)
