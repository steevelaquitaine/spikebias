...
def main(rank, n_ranks):
    ...
    # initialize the data
    max_anr, min_anr = 0, 0
    snr_ns, snr_nv, snr_ne, snr_nb = None, None, None, None
    snr_hv2 = None
    snr_hs2 = None
    ...
    if rank == 0:
        ...
        mean_nb, ci_nb, _ = amp.get_snr_pdfs(snr_nb, bins)
    ...
    ci_nb = comm.gather(ci_nb, root=0)
    ...
    if rank == 0:
        ...
        logger.info(f"sum(mean_nv): {sum(mean_ns)} should be close to 1")
        ...
        # save
        plt.savefig("figures/0_validation/3_anr/fig2q_anr_full_l5.svg", **savefig_cfg)
