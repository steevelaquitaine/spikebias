...
def main(rank, n_ranks):
    ...
    PREP_PATH_ns = cfg_ns["preprocessing"]["output"]["full"][
        "trace_file_path_gain_fitd_adj10perc_less_noise_fitd_int16"
    ]

    # neuropixels (evoked biophysical model)
    cfg_ne, _ = get_config("silico_neuropixels", "npx_evoked").values()
    PREP_PATH_ne = cfg_ne["preprocessing"]["output"]["full"][
        "trace_file_path_gain_fitd_adj10perc_less_noise_fitd_int16"
    ]
    ...
    t0 = time.time()
    logger.info(
        f"Started computing/saving amplitude-to-noise ratios on {n_ranks} nodes"
    )
    ...
    if rank == 0:
        ...
        mean_hv1, ci_hv1, _ = amp.get_snr_pdfs(snr_hv1, bins)
    elif rank == 4:
        mean_hs1, ci_hs1, _ = amp.get_snr_pdfs(snr_hs1, bins)
    ...
    logger.info(
        f"Done gathering stats on master node 0 in {np.round(time.time()-t0, 2)} secs"
    )
    ...
    if rank == 0:
        ...
        logger.info(
            f"Done pooling dense probe depths in {np.round(time.time()-t0, 2)} secs"
        )
        ...
        # save
        plt.savefig("figures/0_validation/3_anr/fig2q_anr_full_l1.svg", **savefig_cfg)
