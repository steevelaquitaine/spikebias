...
def main(rank, n_ranks):
    ...
    if rank == 0:
        ...
        # save/load figure data
        if SAVE_FIG_DATA:
            np.save(fig_data_paths["fig2q_anr_l1"], fig_data)
        elif LOAD_FIG_DATA:
            fig_data = np.load(fig_data_paths["fig2q_anr_l1"], fig_data)
        ...
        # save
        if SAVE_FIG:
            plt.savefig("figures/0_validation/3_anr/fig2q_anr_full_l1_test.svg", **savefig_cfg)
