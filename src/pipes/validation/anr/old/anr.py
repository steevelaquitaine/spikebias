...
Usage:
    ...
    nohup bash -c 'python -u -m src.pipes.validation.anr.anr \
        ...
        --max-duration 1000000000000000000 \
        --layer L1 \
        --save-data-path anrs_l1.npz \
        --save-fig-path fig2q_l1.svg \
        > out_anrs_l1.log 2>&1 && curl -d "Completed pipeline anr-L1" ntfy.sh/Code' > /dev/null 2>&1 &

    # L5 - PID: 3901625
    # - full -> fails
    # - 40 min -> fails
    # - recode to compute site-by-site
    nohup bash -c 'python -u -m src.pipes.validation.anr.anr \
        --recording-path-marques dataset/00_raw/recording_marques_smith --freq-max-marques 14999 \
        --recording-path-npx-spont dataset/00_raw/recording_npx_spont --freq-max-npx-spont 19999 \
        --recording-path-npx-evoked dataset/00_raw/recording_npx_evoked --freq-max-npx-evoked 9999 \
        ...
        > out_anrs_l5.log 2>&1 && curl -d "Completed pipeline anr-L5" ntfy.sh/Code' > /dev/null 2>&1 &
...
def compute_anrs_and_range(
    ...
    # save to disk, free RAM
    np.save(f"{save_anr_path}", anr)
    return min_anr, max_anr


def compute_anrs_and_range_buccino(
...
def main(args):
    ...
    # Pass 1: compute ANRs one at a time, save to disk, track global min/max
    global_min, global_max = np.inf, -np.inf

    configs = [
        ("m", args.recording_path_marques, args.freq_max_marques),
        ("ns", args.recording_path_npx_spont, args.freq_max_npx_spont),
        ("ne", args.recording_path_npx_evoked, args.freq_max_npx_evoked),
        ("d1", args.recording_path_dense_probe, args.freq_max_dense_probe),
        ("h1", args.recording_path_horvath_probe, args.freq_max_horvath_probe),
    ]
    if args.layer == "L5":
        configs.append(("b", args.recording_path_buccino, args.freq_max_buccino))

    # compute anr
    for name, raw_path, freq_max in configs:
        logger.info(f"Computing anr for {name} ...")
        if not name == "b":
            min_anr, max_anr = compute_anrs_and_range(
                layer=args.layer,
                raw_path=raw_path,
                ...
                save_anr_path=f"./anr_{args.layer}_{name}.npy",
            )
        else:
            min_anr, max_anr = compute_anrs_and_range_buccino(
                raw_path=raw_path,
                freq_min=args.freq_min,
                freq_max=freq_max,
                compress_to_int16=args.compress_to_int16,
        ...
        # calculate common min and max anrs
        global_min = min(global_min, min_anr)
        global_max = max(global_max, max_anr)

        # report status
        logger.info(f"Range for {raw_path}: [{min_anr}, {max_anr}]")
        logger.info(f"Done computing anr for {raw_path} ----------")

    # Compute common bins
    steps = (global_max - global_min) / N_BINS
    bins = np.arange(global_min, global_max + steps / 2, steps)
    logger.info(
        f"Global ANR range and common bins: [{global_min}, {global_max}, {bins}]"
    )

    # Pass 2: load saved ANRs one at a time and compute stats
    t0 = time.time()
    logger.info(f"Computing ANR stats ...")

    means, cis, anrs_dict = {}, {}, {}

    for name, raw_path, _ in configs:
        logger.info(f"- stats for {name} ...")
        anr = np.load(f"./anr_{args.layer}_{name}.npy")
        mean, ci, _ = amp.get_snr_pdfs(anr, bins)
        means[name] = mean
        cis[name] = ci
        del anr  # free RAM

    # report status
    logger.info(f"Done computing ANR stats in {np.round(time.time()-t0,2)}")
...
if __name__ == "__main__":
    ...
    # min frequence for band-pass filtering
    parser.add_argument(
        "--freq-min",
        type=int,
        default=None,
        help="freq min of pass filter cutoff",
    )
