...
Usage:
    ...
    sbatch cluster/figures/main/marques/noise_40m_buttw_noise_20_perc_lower.sbatch 
"""
...
# - get silico traces
PreRecording_h_silico = si.load_extractor(PREPRO_SILI)
...
os.makedirs(os.path.dirname(NOISE_SILI_DATAPATH), exist_ok=True)
np.save(NOISE_VIVO_DATAPATH, vivo_noise_by_trace)
np.save(NOISE_SILI_DATAPATH, silico_noise_by_trace)

# check
print("Path preprocessed Marques silico 20 perc. less noise:", PREPRO_SILI)
print("Path Marques noise silico 20 perc. less noise", NOISE_SILI_DATAPATH)