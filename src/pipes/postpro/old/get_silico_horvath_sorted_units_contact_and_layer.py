"""Pipeline to get in silico Horvath sorted units' metadata (takes 5 min)
author: steeve.laquitaine@epfl.ch

Usage:
    sbatch cluster/postpro/get_silico_horvath_sorted_units_metadata.sbatch
...
data_conf_silico_1, _ = get_config(EXPERIMENT, SIMULATION_DATE).values()
...
STUDY_FOLDER_silico_1 = data_conf_silico_1["postprocessing"]["waveform"]["study"]
SORTED_NEURON_METADATA_FILE_1 = data_conf_silico_1["postprocessing"]["sorted_neuron_metadata"]

SIMULATION_DATE = "probe_2"
data_conf_silico_2, _ = get_config(EXPERIMENT, SIMULATION_DATE).values()
...
SIMULATION_DATE = "probe_3"
data_conf_silico_3, _ = get_config(EXPERIMENT, SIMULATION_DATE).values()
...
def get_contact_layers(data_conf:dict):
    """get the layer location of each contact

    Args:
        RAW_HORVATH_1 (str): _description_
