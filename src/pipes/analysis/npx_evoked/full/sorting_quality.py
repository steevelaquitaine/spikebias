...
KS2_5_nb_10m = cfg["sorting"]["sorters"]["kilosort2_5"]["full"]["output"]
KS2_nb_10m = cfg["sorting"]["sorters"]["kilosort2"]["full"]["output"]
...
def main(rank):
    ...
    if rank == 0:
        ...
        df_nb_ks2 = quality.qualify_sorted_units2(
            scores_nb_ks2.values,
            scores_nb_ks2.index.values,
            chance_nb_ks2.values,
            THR_GOOD,
            KS2_nb_10m
        )
        logger.info(f"Quality done on rank {rank} in {np.round(time.time()-t0,2)} secs")     
