...
Usage:
    ...
    sbatch cluster/analysis/npx_evoked/full/code/igeom12.sh
...
SAMPLE_SIZE = 60          # 60; number of units used to calculate information capacity for all conditions
N_NEW = SAMPLE_SIZE       # default: 200; nb of new neural latents resulting from Gaussian random projection ()
SEED = 0                  # reproducibility
BLOCK = 12
N_BOOT = 2               # should take 6 hours
SAMPLE_DIR = np.arange(0, 360, 45) # sample 5 directions out of the 360; (4, 5, 6, fails, 8 worked)
...
def main(rank):
    ...
    if rank == 1:
        ...
        # KS4
        df_k4 = igeom.get_igeom_metrics_bootstrapped_filtered("KS4", K4, GT, **igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
        
    elif rank == 2:
        
        # KS3
        df_k3 = igeom.get_igeom_metrics_bootstrapped_filtered("KS3", K3, GT, **igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
                
    elif rank == 3:
        
        # KS2.5
        df_k25 = igeom.get_igeom_metrics_bootstrapped_filtered("KS2.5", K25, GT, **igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
        
    elif rank == 4:
        
        # KS2
        df_k2 = igeom.get_igeom_metrics_bootstrapped_filtered("KS2", K2, GT,**igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
        
    elif rank == 5:
        
        # KS
        df_ks = igeom.get_igeom_metrics_bootstrapped_filtered("KS", KS, GT, **igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
        
    elif rank == 6:
        
        # KS
        df_hs = igeom.get_igeom_metrics_bootstrapped_filtered("HS", HS, GT, **igeom_params)
        logger.info(f"Done getting info-geom metrics on rank {rank} in {np.round(time.time()-t0,2)} secs")
