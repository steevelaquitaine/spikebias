...
def _neuron_locations_3d(c, target_name):
    """Uses bluepy to get 3D locations of neurons"""
    pos = c.cells.positions(group=target_name)
...
def get_neuron_locations(c, target_name):
    """Return neuron locations in 2D (if flatmap is available) and 3d (via bluepy)"""
    gids, pos3d = _neuron_locations_3d(c, target_name)
    if "atlas" in list(c.config.keys()):
        fm = c.atlas.load_data("flatmap")
        pos2d = apply_flatmap(pos3d, None, fm)
        return gids, pos2d, pos3d
    else:
        warnings.warn("No atlas found in the CircuitConfig, so 2D locations won't be returned")
        return gids, None, pos3d
...
def cluster_by_locations(gids, pos, n_clusters=None, n_per_cluster=None, cluster_seed=0):
    """Perform K-means clustering of fiber locations. (Either `n_clusters` or `n_per_cluster` needs to be specified
    to determine the resulting number of clusters. `pos` can be either 2D or 3D positions.)"""
