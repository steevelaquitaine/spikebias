...
#!/bin/bash -l
#SBATCH -J prepro-for-spikewarp1X                        # job name
...
#SBATCH -o ./cluster/logs/cluster/output/spikewarp/prepro/slurm_nm_%x_id_%A.out   # set log output file path
#SBATCH -e ./cluster/logs/cluster/output/spikewarp/prepro/slurm_nm_%x_id_%A.err   # set log error file path
...
# run preprocessing
srun -n 1 python3.9 -c "from src.pipes.prepro.others.spikewarp.s1X.process import run; run(filtering='butterworth')"