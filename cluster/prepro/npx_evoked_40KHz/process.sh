...
## usage:
##
##      sbatch cluster/prepro/npx_evoked_40KHz/process.sh
##
...
#!/bin/bash -l
#SBATCH -J prep-npx-evoked-40KHz                                # job name
...
#SBATCH -o ./cluster/logs/cluster/output/prepro/npx_evoked_40Khz/%x_id_%A.out   # set log output file path
#SBATCH -e ./cluster/logs/cluster/output/prepro/npx_evoked_40Khz/%x_id_%A.err   # set log error file path
...
# run pipeline
srun -n 1 python3.9 -c "from src.pipes.prepro.npx_evoked_40Khz.process import run; run(filtering='butterworth')"