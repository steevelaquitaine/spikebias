...
##
## duration: 40 min (for 1 hour recording)

## Setup job config

#!/bin/bash -l
#SBATCH -J dense-vivo-probe2                             # name job
...
# run pipeline
srun -n 1 python3.9 -c "from src.pipes.prepro.horvath_vivo.process import run; run(experiment='vivo_horvath', run='probe_2')"